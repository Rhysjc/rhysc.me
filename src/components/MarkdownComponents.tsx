...
const p: React.FC<React.PropsWithChildren> = (props) => {
  const element = Array.isArray(props.children) ? props.children[0] : props.children;

  return typeof element === 'string' ? <p className='text-lg text-zinc-800 dark:text-zinc-100 mb-6 only:mb-0 last:mb-0' {...props} /> : { ...element };
};
